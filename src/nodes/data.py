...
class DataNode(Node):
    ...
    def get_event_by_condition_id(self, condition_id: str, cache: Optional[bool] = configs.CACHE):
        ...
        if cache:
            if not os.path.exists(f'./data/{condition_id}'):
                utility.log(f'Creating a new directory for the condition_id: {condition_id}')
                os.makedirs(f'./data/{condition_id}')
            
            if r.status_code == 200:
                with open(f'./data/{condition_id}/event.json', 'w') as f:
                    utility.log(f'Saving event data for the condition_id: {condition_id}')
    ...
    def get_relevant_news(self, condition_id: str) -> List[str]:
        ...
        if os.path.exists(f'./data/{condition_id}/news.json'):
            utility.log(f'Using locally saved news data for the condition_id: {condition_id}')
            ...
            if not os.path.exists(f'./data/{condition_id}/event.json'):
                utility.log(f'Found no event data for the condition_id: {condition_id}. Fetching from the server')
                event = self.get_event_by_condition_id(condition_id)
            else:
                # Use locally saved event data
                utility.log(f'Using locally saved event data for the condition_id: {condition_id}')
                with open(f'./data/{condition_id}/event.json', 'r') as f:
                    event = json.load(f)
